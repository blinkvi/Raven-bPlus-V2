...
public class AimAssist extends ClientModule {
    ...
    public void update() {
        ...
        if (!weaponOnly.isToggled() || Utils.Player.isPlayerHoldingWeapon()) {
            final RightClicker autoClicker = (RightClicker) Raven.moduleManager.getModuleByClazz(RightClicker.class);
                ...
                if (en != null) {
                    if (blatantMode.isToggled()) {
                        ...
                        if (n > 1.0 || n < -1.0) {
                            ...
                            final double val2 = complimentSpeed + ThreadLocalRandom.current().nextDouble(speed.getInput() - 4.723847, speed.getInput());
    ...
    public Entity getEnemy() {
        ...
        for (final EntityPlayer en : mc.theWorld.playerEntities) {
            if ((!ignoreFriends.isToggled() || !isAFriend((Entity)en)) && en != mc.thePlayer && !en.isDead && (aimInvis.isToggled() || !en.isInvisible()) && mc.thePlayer.getDistanceToEntity((Entity)en) <= distance.getInput() && !bot.bot((Entity)en) && (blatantMode.isToggled() || Utils.Player.fov(en, fov))) {
                return (Entity)en;
            }
        }
        return null;
    ...
    public boolean addFriend(final String name) {
        ...
        for (final Entity entity : mc.theWorld.getLoadedEntityList()) {
            if ((entity.getName().equalsIgnoreCase(name) || entity.getCustomNameTag().equalsIgnoreCase(name)) && !isAFriend(entity)) {
                addFriend(entity);
                found = true;
            }
        }
    ...
    public boolean removeFriend(final String name) {
        ...
        boolean found = false;
        for (final NetworkPlayerInfo networkPlayerInfo : new ArrayList<NetworkPlayerInfo>(mc.getNetHandler().getPlayerInfoMap())) {
            final Entity entity = (Entity)mc.theWorld.getPlayerEntityByName(networkPlayerInfo.getDisplayName().getUnformattedText());
